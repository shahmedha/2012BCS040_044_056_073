...
int main (int argc, char *argv[])
{
	...
	{					// handles cmd-line params
	  ...
	  if (myfile.is_open())
	  {
		...
		n["Author"]=auth;
		n["Price"]=(int)price;
		n["Limit_in_days"]=limit;
		n["Rating"]=rating;
		n["Genre"]=genre;
		n["Edition"]=edition;
		n["Payment_Mode"]=payment_mode;
		n["Year_of_publish"]=year;
		
		cout<<"\nBook Name:"<< book_name<<"  Id:"<<id<<"  Author:" <<auth<<"  Price:"<<price<<" Limit:"<<limit<<" Rating:"<<rating<<" Genre:"<<genre<<" Edition:"<<edition<<" Payment_mode:"<<payment_mode<<" Year:"<<year<<endl;
		siena.publish(n);
		//sleep(1);
		iss.str("");
