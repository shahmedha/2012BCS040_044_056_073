...
public class TestDVDRPDispatcherRemote extends TestCase {
    ...
    RemoteInterestedParty[] ips = new RemoteInterestedParty[serverNo+1];
	...
	for (int i = 0; i < serverNo; i++) {
	    ...
	    PacketReceiver rec = new KAZipPacketReceiver(SENP.DEFAULT_PORT + i);
	    multiReceiver.addDefaultReceiver(rec, 10);
	    dispatchers[i].setChoke(100);
	    dispatchers[i].setReceiver(multiReceiver);
	    dispatchers[i].startHeartbeat();
	    ips[i] = new RemoteInterestedParty(new String(rec.address()),"Client" + i);
	}
	ips[4] = new RemoteInterestedParty(new String(dispatchers[3].listener.address()),"SendOnly");
	...
	pack.cost = 1;
	try {
	    PacketSender sd = psf.createPacketSender(new String(dest.listener.address()));
    ...
    public class RemoteInterestedParty implements Notifiable {

	public int nCount = 0;

	private String id;
		
	SingleNotifiableClient client;

	public RemoteInterestedParty(String server, String id) throws Exception {
	    client = new SingleNotifiableClient(server,this,id);
	    this.id = id;
	}
