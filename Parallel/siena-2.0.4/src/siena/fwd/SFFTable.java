...
class ConstraintIndex<T extends Comparable<? super T>> implements Comparator<T> {
	...
	//System.out.println("In match method of 337");
	if (any_c != null)
	    if (p.process_constraint(any_c)){
		//	System.out.println("In match method of 340");
		return true;}

	SFFConstraint c = eq_map.get(s);
	if (c != null) {
	    if (p.process_constraint(c)){
		//System.out.println("In match method of 346");
		return true;}
	} 
	...
	for(Map.Entry<T,SFFConstraint> entry: ne_map.entrySet()) {
		...
		//System.out.println("In match method of 354");
		    return true;}
	    } else {
	...
	for(SFFConstraint k: gt_map.headMap(s).values()) {
		...
		//System.out.println("In match method of 363");
		return true;}
	}

	for(SFFConstraint k: lt_map.headMap(s).values()) {
	    if(p.process_constraint(k)){
		//System.out.println("In match method of 369");
		return true;}
	}
	return false;
    }
...
class StringIndex extends ConstraintIndex<String> {
	...
	//System.out.println("In match method of 470");	
	if (super.match(s,p)){
		//System.out.println("In match method of 472 StringIndex");
	    return true;}

	for(Map.Entry<String,SFFConstraint> entry: ss_map.entrySet()) {
	    if (s.indexOf(entry.getKey()) >= 0) {
		if(p.process_constraint(entry.getValue())) {
			//System.out.println("In match method of 475 StringIndex");
		    return true;}
	    } 
	}
	for(Map.Entry<String,SFFConstraint> entry: pf_map.entrySet()) {
		...
		if(p.process_constraint(entry.getValue())) {
			//System.out.println("In match method of 482 StringIndex");
		    return true;}
	    }
	}
	for(Map.Entry<String,SFFConstraint> entry: sf_map.entrySet()) {
		...
		if(p.process_constraint(entry.getValue())) {
			//System.out.println("In match method of 489 StringIndex");
		    return true;}
	    } 
	}
	return false;
...
public class SFFTable implements ForwardingTable {
    ...
    public void match(final Message m, final MatchHandler p) {
	...
	final IFMask mask = new BitVectorMask(if_count);
	final ConstraintProcessor cp = new ConstraintProcessor(if_count, p, mask, m);
	// 
	...
	int threadNum = attributes.size();
	//System.out.println("SIZE:"+threadNum);
    ...
    public void parallel(Attribute a,IFMask mask,ConstraintProcessor cp)
    {	 
